...
# Default headers to skip - these are typically auto-generated by browsers/clients
...
# Headers that should always be included regardless of filtering
...
class TreeGeneral:
	def __init__(self, request_data, url, custom_skip_headers=None, enable_header_filtering=True):
		rd = request_data.split(u"\n")
		self.method = self._get_method(rd)
		self.enable_header_filtering = enable_header_filtering
		self.skip_headers = self._get_skip_headers(custom_skip_headers)
	...
	def _get_skip_headers(self, custom_skip_headers):
		if not self.enable_header_filtering:
			return []
		if custom_skip_headers is not None:
			return custom_skip_headers
		return DEFAULT_SKIP_HEADERS
	...
	def _get_headers(self, rd):
		...
		for header_line in headers_raw.split(u"\n"):
			...
			# Skip header if filtering is enabled and header should be skipped
			if (self.enable_header_filtering and 
			    header_name in self.skip_headers and 
			    header_name not in ALWAYS_INCLUDE_HEADERS):
				continue
			header_value = u":".join(
